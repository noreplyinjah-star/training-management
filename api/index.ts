...
const logActivity = async (userId: any, username: any, action: any, details: any) => {
  if (!supabase) return;
  await supabase.from('activity_logs').insert([{ user_id: userId, username, action, details }]);
};

const seedAdmin = async () => {
  if (!supabase) return;
  try {
    const { data: adminExists } = await supabase.from('users').select('*').eq('username', 'admin').single();
    if (!adminExists) {
...
app.post('/api/login', async (req, res) => {
  ...
  if (!supabaseUrl) return res.status(500).json({ success: false, message: 'Supabase URL not configured' });
...
app.post('/api/programs', async (req, res) => {
  ...
  if (!supabaseUrl) return res.json({ id: null });
  
  const { data, error } = await supabase.from('programs').insert([{ name, end_date }]).select().single();
  if (!error && data) {
...
app.delete('/api/programs/:id', async (req, res) => {
  ...
  if (!supabaseUrl) return res.json({ success: true });
  
  const { data: program } = await supabase.from('programs').select('name').eq('id', req.params.id).single();
...
app.get('/api/programs/:id/topics', async (req, res) => {
  if (!supabaseUrl) return res.json([]);
  const { data: topics } = await supabase.from('topics').select('*').eq('program_id', req.params.id);
  res.json(topics || []);
});

app.post('/api/programs/:id/topics', async (req, res) => {
  const { names, userId, username } = req.body;
  if (!supabaseUrl) return res.json({ success: true });
  
  const { data: program } = await supabase.from('programs').select('name').eq('id', req.params.id).single();
  const topicsToInsert = names.map((name: string) => ({ program_id: req.params.id, name: name.trim() }));
...
app.delete('/api/topics/:id', async (req, res) => {
  ...
  const { data: topic } = await supabase.from('topics').select('name').eq('id', req.params.id).single();
...
app.get('/api/programs/:id/trainees', async (req, res) => {
  if (!supabaseUrl) return res.json([]);
  const { data: trainees } = await supabase.from('trainees').select('*').eq('program_id', req.params.id);
  res.json(trainees || []);
});

app.post('/api/programs/:id/trainees', async (req, res) => {
  const { trainees, userId, username } = req.body;
  if (!supabaseUrl) return res.json({ success: true });
  
  const { data: program } = await supabase.from('programs').select('name').eq('id', req.params.id).single();
  const traineesToInsert = trainees.map((t: any) => ({ program_id: req.params.id, name: t.name.trim(), email: t.email?.trim() || '' }));
...
app.delete('/api/trainees/:id', async (req, res) => {
  ...
  const { data: trainee } = await supabase.from('trainees').select('name').eq('id', req.params.id).single();
...
app.get('/api/grades/:programId', async (req, res) => {
  ...
  const { data: grades, error } = await supabase
    .from('grades')
    .select(`*, trainees!inner(name, program_id), topics!inner(name)`)
    .eq('trainees.program_id', req.params.programId);
    
  if (error) return res.json([]);
  const formattedGrades = grades.map((g: any) => ({ ...g, trainee_name: g.trainees.name, topic_name: g.topics.name }));
...
app.post('/api/grades', async (req, res) => {
  ...
  await supabase.from('grades').upsert({ trainee_id, topic_id, score, status }, { onConflict: 'trainee_id,topic_id' });
...
app.get('/api/trainees/:id/report', async (req, res) => {
  if (!supabaseUrl) return res.json({ error: 'Supabase not configured' });
  
  const { data: trainee } = await supabase.from('trainees').select('*').eq('id', req.params.id).single();
  if (!trainee) return res.status(404).json({ error: 'Trainee not found' });

  const { count: totalTopics } = await supabase.from('topics').select('*', { count: 'exact', head: true }).eq('program_id', trainee.program_id);
  const { data: topics } = await supabase.from('topics').select('*').eq('program_id', trainee.program_id);
  const { data: grades } = await supabase.from('grades').select('*').eq('trainee_id', req.params.id);
    
  const reportGrades = (topics || []).map((top: any) => {
    const grade = (grades || []).find((g: any) => g.topic_id === top.id);
    return { ...grade, topic_name: top.name, topic_id: top.id, trainee_id: trainee.id };
  });
  ...
  res.json({ trainee, stats: { totalTopics: totalTopics || 0, evaluated, passed, failed, incomplete }, details: reportGrades });
});

app.get('/api/staff', async (req, res) => {
  if (!supabaseUrl) return res.json([]);
  const { data: staff } = await supabase.from('users').select('id, username, role');
  res.json(staff || []);
});
...
app.post('/api/staff', async (req, res) => {
  ...
  const { data, error } = await supabase.from('users').insert([{ username, password, role: role || 'staff' }]).select().single();
  if (error) {
...
app.delete('/api/staff/:id', async (req, res) => {
  ...
  const { data: staff } = await supabase.from('users').select('username').eq('id', req.params.id).single();
